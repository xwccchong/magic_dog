...
from unitree_nav_v6 import AsyncNavigationManager
...
# 基础导航函数
async def send_nav_goal(poses_list, location_name="", photo_at_each_point=True, wait_stable_time=4.0):
    """发送导航目标

    Args:
        poses_list: 目标位姿列表
        location_name: 目标位置名称
        photo_at_each_point: 是否在每个点拍照（逐点导航模式）
        wait_stable_time: 到达后等待稳定时间（秒）
    """
    ...
    try:
        if photo_at_each_point and len(poses_list) > 1:
            # 逐点导航模式：每到一个点等待稳定后拍照
            print(f"📸 逐点导航模式: 每个点等待 {wait_stable_time}s 稳定后拍照")
            nav.start_async_with_photo(
                pose_list=poses_list,
                speed=0.5,
                wait_stable_time=wait_stable_time,
                photo_on_arrival=True
            )
        else:
            # 普通导航模式：一次性导航完所有点
...
async def execute_tasks():
    ...
    if inspection_results and sub_task.task_type == TaskType.NAVIGATE and sub_task.data.get("location") == "起点":
