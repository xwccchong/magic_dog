...
# 👇 新增：系统模式全局变量
current_system_mode = "跑腿模式"  # 默认启动时为跑腿模式
...
示例1（明确了送达目的地）："把纸巾从洗手台拿来放到办公室" 
输出：{"item": "纸巾", "action_plan": [{"action": "navigate", "location": "门口"}, {"action": "squat"}, {"action": "send_grasp_msg", "item": "纸巾"}, {"action": "wait_for_back"}, {"action": "stand_up"}, {"action": "navigate", "location": "办公室"}, {"action": "play_audio"}]}

示例2（未说明送达地）："去帮我到吧台拿瓶水"
输出：{"item": "水", "action_plan": [{"action": "navigate", "location": "吧台"}, {"action": "squat"}, {"action": "send_grasp_msg", "item": "水"}, {"action": "wait_for_back"}, {"action": "stand_up"}, {"action": "navigate", "location": "__DEPARTURE__"}, {"action": "play_audio"}]}

说明：
1. 包含抓取要求时，必须严格执行【到达地点 -> 蹲下 -> 要求抓取 -> 等待 -> 📌必须先站立(stand_up) -> 再导航去目标点 -> 播放音效】的绝对顺序，绝不能漏掉起身！
...
3. 必须返回 JSON 格式，包含 "chat_reply" 和 "action_plan" 两个字段。
4. ⚠️ 严格区分闲聊与动作：
   - 如果用户只是普通的问答或闲聊（例如：“今天天气怎么样”、“我晚上吃什么”、“你真聪明”），绝对不需要做任何动作，此时 "action_plan" 必须严格为空列表 []。
   - 只有在用户明确要求你做动作或表演（例如：“走两步”、“转圈”、“趴下”、“伸懒腰”、“打个招呼”）时，才在 "action_plan" 中加入对应的 action。
...
【输出示例】：
- 闲聊情景1：用户输入“今天天气真好啊” 
  输出: {"chat_reply": "是呀，今天阳光明媚，很适合出去玩呢！", "action_plan": []}
- 闲聊情景2：用户输入“晚上吃什么？” 
  输出: {"chat_reply": "不如去吃顿好吃的尤加利叶火锅吧！", "action_plan": []}
- 动作情景3：用户输入“给大家表演个转圈和伸懒腰” 
  输出: {"chat_reply": "好嘞，看我的！", "action_plan": [{"action": "rotate"}, {"action": "stretch"}]}

⚠️【重要容错规则】：
如果用户要求你去某个地点跑腿拿东西（如去前台拿水、跨房间送物），这超出了表演模式的职责！
请立刻停止规划，直接输出如下JSON以报错返回：
...
def send_keyword_notification(final_result):
    ...
    # ================= 模式切换拦截器 =================
    if "表演模式" in final_result:
        current_system_mode = "表演模式"
        print("\n🔄 系统已切换至：🎭 表演模式")
        threading.Thread(target=speak_with_cherry, args=("已切换到表演模式，您可以随时找我聊天或看表演哦。",), daemon=True).start()
        return
    elif "跑腿模式" in final_result:
        current_system_mode = "跑腿模式"
        print("\n🔄 系统已切换至：🚀 跑腿模式")
        threading.Thread(target=speak_with_cherry, args=("已切换到跑腿模式，您可以吩咐我去跑腿拿东西了。",), daemon=True).start()
        return
    # ================================================

    llm_result = parse_with_llm(final_result)
    
    # 兼容处理大模型偶尔单纯输出List的情况
    ...
    if "action_plan" in llm_result:
        ...
        for step in llm_result["action_plan"]:
            if isinstance(step, str):
                clean_plan.append({"action": step})  # 把 "hello" 强行包成 {"action": "hello"}
            elif isinstance(step, dict):
                clean_plan.append(step)              # 标准字典原样放行
        llm_result["action_plan"] = clean_plan
        
    # 根据当前模式执行不同的播报和任务分发策略
    if current_system_mode == "跑腿模式":
        
        if llm_result.get("error") == "mode_mismatch":
            warn_text = "如果要聊天或看表演，请先对我说：切换到表演模式哦。"
            print(f"⚠️ 越界指令被拦截：提醒用户切换模式")
        ...
        if llm_result.get("error") == "mode_mismatch":
            warn_text = "如需帮您跑腿拿东西，请先对我说：切换到跑腿模式哦。"
            print(f"⚠️ 越界指令被拦截：提醒用户切换模式")
            threading.Thread(target=speak_with_cherry, args=(warn_text,), daemon=True).start()
            return
        # 获取大模型的智能聊天回复，如果没有就给个保底话术
        chat_text = llm_result.get("chat_reply", "收到啦！")
        action_plan = llm_result.get("action_plan", [])
        
        def speak_then_send_perf():
            try:
                print(f"🎤 聊天回复: {chat_text}")
                speak_with_cherry(chat_text)
            except Exception as e:
                print(f"❌ 播报失败: {e}")
            finally:
                if action_plan:
                    # 如果这句聊天还有附带动作，发给客户端让狗动一动
        ...
        threading.Thread(target=speak_then_send_perf, daemon=True).start()
