...
from task_manager_v4 import TaskManager, TaskType,TaskStatus
...
location_poses_mapping = {
    "洗手台": [
        (4.56139, 0.939847, 0.0609855, 0.0171799 , 0.11993 , -0.161487, 0.97941),
        (6.3713, -4.14591, 0.0433416 , 0.0744915,0.111515, -0.631877, 0.763379)
    ],
    "办公室": [
        # (0.00240726, -0.514558, 0.0382567, 0.0061988, -0.137903, -0.00690807, 0.990402),
        (-1.33403, 1.91519, 0.035477, 0.0882469  , -0.0550041 , -0.857748, -0.503444)
    ],
    "吧台": [
        (1.59986 , -2.0031, 0.0466795, 0.0792104, 0.102109,-0.616705, 0.776514)
    ],
    "初始位置": [
        (-0.0519591, -0.347845, 0.0302814, -0.0469132,0.0962213, 0.19407, 0.975)
    ],
    "门口": [
        (5.45127, 1.15249, 0.0644253, -0.0254704, 0.131708, 0.0980803, 0.986096)
    ],
    "雨伞架": [
...
async def execute_sub_task(sub_task):
    ...
    elif task_type == TaskType.WALK:
        await execute_action_script("walk.py", "走路")
        task_manager.complete_current_sub_task()
