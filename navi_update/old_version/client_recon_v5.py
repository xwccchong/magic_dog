...
# ===== 全局配置 -*-
WEBSOCKET_URI = "ws://10.217.189.156:8765"  # 连接机器狗8765端口
# 手动修改一下路径
...
location_poses_mapping = _client_cfg["sf_location_poses_mapping"]
LOCATION_INTRO_MAP = _client_cfg["location_intro_map"]
...
async def execute_sub_task(sub_task):
    ...
    # ===== 以下为原地操作动作，不需要记录初始位置 =====
        ...
        intro_text = LOCATION_INTRO_MAP.get(location, "")
        if intro_text:
            print(f"📢 定点介绍 [{location}]: {intro_text}")
            threading.Thread(target=speak_with_cherry, args=(intro_text,), daemon=True).start()
        else:
            print(f"⚠️ 地点 [{location}] 无预设介绍内容，跳过播报")
        task_manager.complete_current_sub_task()
        ...
        # 使用异步等待，不会阻塞 WebSocket 监听或其他后台任务 
        ...
        if len(rgbs) > 0:
            ...
            if result:
                result["location"] = data.get("location", "未指定")
