...
#                 break
#     # 👇 新增：检查是否需要自动添加默认取物地点
#     detected_position_names = [pos for _, pos in detected_positions]
    
#     # 如果物品有默认地点，且该地点不在检测到的地点列表中
#     if detected_item_name in ITEM_DEFAULT_LOCATION:
#         default_location = ITEM_DEFAULT_LOCATION[detected_item_name]
        
#         # 检查是否已经有取物地点（非送达地点）
#         has_pickup_location = False
#         for _, position in detected_positions:
#             if position != delivery_location:
#                 has_pickup_location = True
#                 break
        
#         # 如果没有取物地点，或者只有送达地点，则添加默认地点
#         if not has_pickup_location or (len(detected_positions) == 1 and delivery_location):
#             print(f"🔗 自动绑定: '{detected_item_name}' -> 默认取物地点 '{default_location}'")
#             # 插入到地点列表最前面（作为取物地点）
#             detected_positions.insert(0, (-1, default_location))  # -1 表示最前面
...
# 系统提示词：告诉大模型它的任务和输出格式
LLM_SYSTEM_PROMPT = """你是一个机器人助手的指令解析器。
用户会用自然语言告诉机器人去哪里取什么物品，然后送到哪里。

你需要从用户的话中提取：
1. item: 要取的物品（只能是以下之一：水、纸、糖、咖啡、雨伞、外卖、饮料）
2. pickup_location: 取物品的地点（只能是以下之一：前台、办公室、吧台、初始位置、门口、雨伞架）
   - 如果用户没有说取物地点，填 null
3. delivery_location: 送达地点（只能是以下之一：前台、办公室、吧台、初始位置、门口、雨伞架）
   - 如果用户没有说送达地点，填 null

只输出 JSON，不要有任何其他文字，格式如下：
{"item": "水", "pickup_location": "前台", "delivery_location": "办公室"}

例子：
- "帮我去前台拿水" → {"item": "水", "pickup_location": "前台", "delivery_location": null}
- "我要喝水" → {"item": "水", "pickup_location": null, "delivery_location": null}
- "把外卖从门口拿来放到办公室" → {"item": "外卖", "pickup_location": "门口", "delivery_location": "办公室"}
- "去把雨伞给我拿过来" → {"item": "雨伞", "pickup_location": null, "delivery_location": null}
- "帮我把咖啡送到前台" → {"item": "咖啡", "pickup_location": null, "delivery_location": "前台"}
"""
...
def parse_with_llm(text):
    ...
    try:
        # 👇 使用 OpenAI SDK 调用，不再用 requests.post
        ...
        delivery = parsed.get("delivery_location")
        
        if item not in set(items):
            print(f"   ⚠️ 大模型返回的物品 '{item}' 不在支持列表中")
            return None
        
        if pickup and pickup not in set(positions):
            print(f"   ⚠️ 取物地点 '{pickup}' 不在支持列表中，置为 null")
            pickup = None
        
        if delivery and delivery not in set(positions):
            print(f"   ⚠️ 送达地点 '{delivery}' 不在支持列表中，置为 null")
            delivery = None
        
        print(f"   ✅ 解析结果: 物品={item}, 取物={pickup}, 送达={delivery}")
        return {
            "item": item,
...
def send_keyword_notification(final_result):
    ...
    if llm_result:
        ...
        pickup_location = llm_result["pickup_location"]
        delivery_location = llm_result["delivery_location"]
        
        # 如果没有取物地点，使用默认地点
        if pickup_location is None and item_name in ITEM_DEFAULT_LOCATION:
            pickup_location = ITEM_DEFAULT_LOCATION[item_name]
            print(f"   🔗 使用默认取物地点: {item_name} → {pickup_location}")
        
        # 构建 locations 列表
        locations = []
        if pickup_location:
            locations.append(pickup_location)
        if delivery_location and delivery_location != pickup_location:
    ...
    # else:
    #     # ===== 降级：使用原始关键词匹配 =====
    #     print("⚠️ 大模型解析失败，降级使用关键词匹配...")
    #     keywords, item_name, delivery_location = extract_keywords(final_result)
        
    #     if keywords is None or len(keywords) < 2:
    #         print("❌ 关键词匹配也失败，不发送消息")
    #         return
    ...
    # 播放出发音效
    audio_file_map = {
        "水": "/home/unitree/dky/go2_python_sdk2/data/wav_files/lc_water_start.wav",
        ...
        "雨伞": "/home/unitree/dky/go2_python_sdk2/data/wav_files/lsl_umbrella_start.wav",
        "外卖": "/home/unitree/dky/go2_python_sdk2/data/wav_files/lc_takeout_start.wav",
