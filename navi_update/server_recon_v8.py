...
# 1. 获取当前脚本所在目录的父目录 (即 /home/unitree/unitree_sdk2_python)
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
# 2. 定位到 agent 目录
_AGENT_DIR = os.path.join(_PROJECT_ROOT, "agent")
# 3. 将 agent 目录加入 sys.path，这样才能正确识别 agent 文件夹下的 config 包
if _AGENT_DIR not in sys.path:
    sys.path.insert(0, _AGENT_DIR)
from config.config_loader import load_config
config = load_config(prompt_filename="llm_prompt_v2.txt")
...
# ===== v8 新增：巡检历史记录 =====
inspection_history = []  # 存储从 Client 接收的巡检历史，上限为1条
...
# LLM Prompt 从配置文件加载（不修改原始prompt）
LLM_PROMPT_UNIVERSAL = config.llm_prompt
print(LLM_PROMPT_UNIVERSAL)

# 异常检测 Prompt 从配置文件加载
# AD_PROMPT = config.ad_prompt
...
def save_inspection_to_server_history(data):
    """保存从 Client 接收的巡检结果到服务端历史记录"""
    global inspection_history
    record = {
        "timestamp": data.get("timestamp", get_formatted_timestamp()),
        "results": data.get("data", []),
        "summary": data.get("summary", "")
    }
    # 清空历史，只保留最新一条
    inspection_history = [record]
    print(f"📝 巡检结果已存入服务端记忆: {record['timestamp']}")
    print(f"   📊 巡检点位数: {len(record['results'])}")
    print(f"   📢 总结: {record['summary'][:100]}..." if len(record['summary']) > 100 else f"   📢 总结: {record['summary']}")

def get_inspection_history_context():
    """获取巡检历史的上下文字符串，用于注入 LLM prompt"""
    global inspection_history
    if not inspection_history:
        return ""

    record = inspection_history[0]  # 只有最新一条
    context = f"\n\n【最近一次巡检记录】（{record['timestamp']}）:\n"
    context += f"巡检总结: {record['summary']}\n"
...
def exhibition_loop():
    ...
    # example："小伙伴儿们，我是来自上海码极客可爱的考拉二号，快来找我玩儿吧～"
    ...
    async def handler(websocket):
        ...
        try:
            async for message in websocket:
                # 尝试解析 JSON，检查是否为照片消息或巡检结果
                try:
                    data = json.loads(message)

                    # ===== v8 新增：处理巡检结果消息 =====
                    if data.get("type") == "inspection_result":
                        save_inspection_to_server_history(data)
                        continue
                    ...
                    if data.get("type") == "photo":
                        ...
                        with open(save_path, 'wb') as f:
                            f.write(img_data)
                        print(f"📸 RGB照片已保存: {save_path}")

                        # 解码并保存深度图（如果存在）
                        if "depth_data" in data and data["depth_data"]:
                            depth_data = base64.b64decode(data["depth_data"])
                            depth_filename = f"depth_{int(timestamp)}.png"
...
def parse_with_llm(text):
    """使用大模型解析自然语言并直接生成执行规划（v8: 注入巡检历史）"""
    print(f"\n🤖 规划大脑思考中: '{text}'")
    import time
    start_time = time.time()

    try:
        # ===== v8 新增：构建带巡检历史的增强 prompt =====
        history_context = get_inspection_history_context()
        enhanced_system_prompt = LLM_PROMPT_UNIVERSAL + history_context
        ...
        try:
            ...
            if user_input.lower() == 'h':
                ...
                if inspection_history:
                    record = inspection_history[0]
                    print(f"   时间: {record['timestamp']}")
                    print(f"   总结: {record['summary']}")
                    if record['results']:
                        print(f"   详细结果:")
                        for i, result in enumerate(record['results'], 1):
                            print(f"      {i}. {result}")
...
def main():
    ...
    print("\n" + "=" * 50)
    print("🎙️  实时语音识别系统 - YL200S麦克风版本 (v8 带巡检记忆)")
    print("=" * 50)
    print("\n📌 使用说明:")
