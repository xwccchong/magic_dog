# -*- coding: utf-8 -*-
"""
实时语音识别 - Client 接收端 (v8 - 带巡检记忆)
需要安装: pip install websockets
使用方式: python client_recon_v8.py
连接到服务器并接收消息
新增功能：巡检结果记忆存储，支持历史查询
"""
...
from task_manager_recon_v4 import TaskManager, TaskType,TaskStatus
...
from actions.photo_capture import execute as _photo_capture
from actions.anomaly_detection import execute as _anomaly_detection
from actions.anomaly_detection_multi import execute as _anomaly_detection_multi
init_dashscope_api_key()

# ===== 全局配置 -*-
WEBSOCKET_URI = "ws://172.50.1.125:8765"  # 连接机器狗8765端口
# 手动修改一下路径
_client_config_path = "/home/unitree/unitree_sdk2_python/agent/config/client_config.json"
...
# 注意：原始 unitree_nav.py 不支持逐点拍照，仅使用基础导航功能

global_websocket = None # 全局 websocket 引用（用于接收 back 消息）
departure_position = None # 全局变量：存储出发位置
ignore_arrived_until = 0.0

# 巡检结果收集（静默模式，任务栈清空后统一总结播报）
inspection_results = []

# ===== v8 新增：巡检历史记录 =====
inspection_history = []  # 存储历史巡检结果，上限为1条
MAX_INSPECTION_HISTORY = 1  # 只保留最新一条

# 从配置文件加载物品映射和地点坐标
with open(_client_config_path, 'r', encoding='utf-8') as _f:
    _client_cfg = json.load(_f)

item_mapping = _client_cfg["item_mapping"]
location_poses_mapping = _client_cfg["location_poses_mapping"]
...
# 下面两个没有调用execute_action_script，是因为还只是单独的函数文件
async def execute_photo_capture(cam, global_websocket, data=None):
    """拍照动作：拍照并发送到服务端"""
    # 确保 data 不为 None
    if data is None:
        data = {}
    await _photo_capture(cam, global_websocket, data)

async def execute_anomaly_detection(cam, global_websocket, data=None):
    """异常检测动作：拍照 + LLM 分析 + 记录结果"""
    global inspection_results
    print("🔍 异常检测: 等待 3 秒让底盘稳定...")
    await asyncio.sleep(3)
    # 确保 data 不为 None
    if data is None:
        data = {}
    # 单视角
    # await _anomaly_detection(cam, global_websocket, data, inspection_results)
    # 多视角
    await _anomaly_detection_multi(cam, global_websocket, data, inspection_results)
...
async def send_inspection_result_to_server(result_data, summary):
    """将巡检结果发送到 Server 端存储"""
    ...
    if global_websocket:
        ...
        try:
            ...
            print(f"📤 巡检结果已发送到服务端记忆系统")
        except Exception as e:
            print(f"❌ 发送巡检结果失败: {e}")

def save_inspection_to_history(results, summary):
    """保存巡检结果到历史记录（限制为最新1条）"""
    ...
    # 清空历史，只保留最新一条
    inspection_history = [record]
    print(f"📝 巡检结果已存入本地历史记录: {record['timestamp']}")
...
async def execute_sub_task(sub_task):
    ...
    elif task_type == TaskType.UNIVERSAL_PLAY_AUDIO:
        print("介绍内容：", data)
        text_to_speak = data.get("text", "请提供播放内容")
        threading.Thread(target=speak_with_cherry, args=(text_to_speak,), daemon=True).start()
        task_manager.complete_current_sub_task()

    elif task_type == TaskType.CAPTURE_PHOTO:
        await execute_photo_capture(cam, global_websocket, data)
        task_manager.complete_current_sub_task()

    elif task_type == TaskType.ANOMALY_DETECTION:
        await execute_anomaly_detection(cam, global_websocket, data)
        task_manager.complete_current_sub_task()
...
async def execute_tasks():
    ...
    # 任务栈清空: 如果有巡检结果, 生成总结并播报
    if inspection_results and sub_task and sub_task.task_type == TaskType.NAVIGATE and sub_task.data.get("location") == "起点":
        print(f"\n📊 巡检完成，共 {len(inspection_results)} 个点位，正在生成总结...")

        from agent.utils.anomaly_detection import summarize_inspection

        summary = summarize_inspection(inspection_results)
        if summary:
            print(f"   📢 巡检总结: {summary}")
            threading.Thread(target=speak_with_cherry, args=(summary,), daemon=True).start()

            # ===== v8 新增：保存巡检结果到历史 =====
            save_inspection_to_history(inspection_results, summary)
            # 发送到服务端
            await send_inspection_result_to_server(inspection_results, summary)
