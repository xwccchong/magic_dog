...
class MainTask:
    """主任务"""
    def __init__(self, item: str, locations: List[str], task_id: int, 
                 delivery_location: str = None, departure_position: Dict = None):
        self.task_id = task_id
        self.item = item
        self.locations = locations
        self.delivery_location = delivery_location
        self.departure_position = departure_position  # 👈 新增：出发位置
    ...
    def _build_sub_tasks(self):
        ...
        # 4. 如果只有一个地点，自动添加返回出发点
        if len(self.locations) == 1:
            # 👇 修改：使用动态出发位置
            if self.departure_position:
                print(f"   自动添加返回 '出发点' 任务")
                self.sub_tasks.append(SubTask(
                    TaskType.NAVIGATE,
                    {
                        "location": "__DEPARTURE__",  # 👈 特殊标记
                        "purpose": "返回",
                        "item": self.item,
                        "departure_position": self.departure_position  # 👈 保存出发位置
                    }
                ))
            else:
        ...
        # 5. 👇 新增：如果没有送达地点且有多个位置，最后添加返回出发点
        elif self.delivery_location is None and len(self.locations) >= 1:
            # 检查最后一个任务是否已经是送物品
            if self.sub_tasks[-1].data.get("purpose") == "送物品":
                # 送物品后返回出发点
                if self.departure_position:
                    print(f"   自动添加返回 '出发点' 任务")
                    self.sub_tasks.append(SubTask(
    ...
    def _get_obj_name(self, item: str) -> str:
        """获取物品对应的对象名称"""
        mapping = {"水": "bottle", "纸": "tissue", "糖": "cup", "雨伞": "red umbrella", "咖啡": "brown bottle","外卖":"brown bag","饮料":"white bottle"}
...
class TaskManager:
    ...
    def add_task(self, item: str, locations: List[str], 
                 delivery_location: str = None, departure_position: Dict = None) -> int:
        """
        添加新任务到预备栈
        ...
        with self.lock:
            self.task_counter += 1
            task = MainTask(item, locations, self.task_counter, 
                          delivery_location, departure_position)
            ...
            if departure_position:
                print(f"   返回位置: ({departure_position['x']:.3f}, {departure_position['y']:.3f}) 📍")
            print(f"   子任务数: {len(task.sub_tasks)}")
            
            return task.task_id

    def interrupt_and_execute(self, item: str, locations: List[str], 
                              delivery_location: str = None, 
                              departure_position: Dict = None) -> int:
        """
        中断当前任务，执行紧急任务
        ...
        with self.lock:
            ...
            # 2. 添加紧急任务并立即加载到执行栈
            task_id = self.add_task(item, locations, delivery_location, departure_position)
    ...
    def get_next_sub_task(self) -> Optional[SubTask]:
        ...
        with self.lock:
            ...
            if self.execution_stack:
                sub_task = self.execution_stack[-1]
                return sub_task
