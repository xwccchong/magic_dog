...
from task_manager_copy import TaskManager, TaskType
...
async def get_current_position():
    """
    获取机器人当前位置
    使用 rosrun tf tf_echo /map /base_link 获取位置信息
    ...
    command = (
        ...
        "timeout 2 rosrun tf tf_echo /map /base_link 2>/dev/null | head -5"
    )
    ...
    try:
        ...
        # 解析 Translation: [x, y, z]
        trans_match = re.search(r'Translation:\s*\[([-\d.]+),\s*([-\d.]+),\s*([-\d.]+)\]', output)
        # 解析 Rotation: in Quaternion [qx, qy, qz, qw]
        rot_match = re.search(r'in Quaternion\s*\[([-\d.]+),\s*([-\d.]+),\s*([-\d.]+),\s*([-\d.]+)\]', output)
...
async def execute_sub_task(sub_task):
    ...
    if sub_task.task_type == TaskType.NAVIGATE:
        ...
        if location == "__DEPARTURE__":
            # 使用记录的出发位置
            goal_config = sub_task.data.get("departure_position")
            if goal_config:
                location_name = "出发点"
                print(f"📍 使用记录的出发位置: ({goal_config['x']:.3f}, {goal_config['y']:.3f})")
            else:
                # 回退到初始位置
                print("⚠️ 出发位置未记录，使用 '初始位置'")
                goal_config = location_mapping.get("初始位置")
                location_name = "初始位置"
        elif location in location_mapping:
            goal_config = location_mapping[location]
        ...
        # 发送导航目标
        await send_nav_goal(goal_config, location_name)
        
        # 如果有暂停的任务，说明是中断任务，需要发送 resume
                    ...
                    try:
                        ...
                        if message == "arrived":
                            ...
                            if current_sub and current_sub.task_type == TaskType.NAVIGATE:
                                ...
                                # 👇 修复：在分支开始前就获取 item，确保每个分支都能正确使用
